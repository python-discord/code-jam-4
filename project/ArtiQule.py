    ...
    def Setup(self):
            ...
            def mix_color(self, tool):
                if isinstance(type(tool), ColorTool):
                    if not ((tool.r and tool.g and tool.r) and self.t):
                        ...
                        mixedColor = QColor(
                            self.r - (max(self.r, tool.r//4) -
                                      min(self.r, tool.r//4)),
                            self.g - (max(self.g, tool.g//4) -
                                      min(self.g, tool.g//4)),
                            self.b - (max(self.b, tool.b//4) -
                                      min(self.b, tool.b//4)),
                            255
                            )
        ...
        class ColorTool(Tool):
            def __init__(self, toolName, brushSize, paintPattern=None,
                ...
                self.r = r   # TODO: duration--, transparancy--
                self.g = g
                self.b = b
                self.t = 0
                self.color = QColor(self.r,
        ...
        #TODO: the two paintbrushes should drip self.color
        # when not mousePressEvent
        ...
        for Color, pallette in zip(colors, pallettes):
            pallette.setStyleSheet("QToolButton{background-color:rgb{color}}"\
